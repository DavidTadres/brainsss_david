...
for date in dates:
    ...
    for current_fly in fly_folders:
        print(current_fly)
        #directory = os.path.join(dataset_path, fly)
        fly_directory = Path(dataset_path, current_fly)
                ...
                for tseries_folder in Path(func_dir).iterdir():
                    if 'TSeries' in tseries_folder.name:
                        ...
                        for nii in channel_1_list:
                            start = nii.rindex(
                                's') + 1  # +1 to get indexing right (want to start with number) (rindex gives index of last occurance)
                        ...
                        for i in sorted_channel_1_list:
                            print('ch1 file: ', i[0])
                            brain_ch1 = np.asarray(nib.load(Path(tseries_folder, i[0])).get_data(), dtype='uint16')
                        ...
                        for i in sorted_channel_2_list:
                            # print('ch2 file: ', i)
                            brain_ch2 = np.asarray(nib.load(Path(tseries_folder, i[0])).get_data(), dtype='uint16')
